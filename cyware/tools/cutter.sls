...
# Notes: If you're planning to use Cutter when running cyware as a Docker container, you'll need to include the `--privileged` parameter when invoking the cyware distro image in Docker.

cyware-tools-cutter-source:
  file.managed:
...
cyware-tools-cutter-icon:
  file.managed:
    ...
    - makedirs: True
    - watch:
      - file: cyware-tools-cutter-source