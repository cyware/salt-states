...
cyware-tools-shellcode2exe-bat-wrapper:
  file.managed:
    - name: /usr/local/bin/shellcode2exe.bat
    - mode: 755
    - watch:
      - git: cyware-tools-shellcode2exe-bat
    - contents:
      - '#!/bin/bash'
      - cd /usr/local/shellcode2exe-bat && wine cmd /c "/usr/local/shellcode2exe-bat/shellcode2exe.bat ${*}"

cyware-tools-shellcode2exe-bat-permissions:
  file.directory:
    - name: /usr/local/shellcode2exe-bat
    - dir_mode: 777
    - watch:
      - file: cyware-tools-shellcode2exe-bat-wrapper
